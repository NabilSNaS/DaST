...
class DCGANGenerator(nn.Module):
    def __init__(self, nz=128, ngf=64, nc=3):
        ...
        self.main = nn.Sequential(
            ...
            nn.Tanh()  # Or use nn.Sigmoid() if your model expects [0, 1] input
        )
...
netG = DCGANGenerator(nz=128).cuda()
...
for epoch in range(opt.niter):
    ...
    for ii in range(batch_num):
        ...
        with torch.no_grad():
            ...
            data_resized = F.interpolate(data, size=(224, 224), mode='bilinear', align_corners=False)
            outputs = original_net(data_resized)
            cnt += 1
            _, label = torch.max(outputs.data, 1)
            outputs = F.softmax(outputs, dim=1)
# Get predictions from the substitute model
